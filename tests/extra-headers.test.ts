...
const extraHeaders = { 'X-Title': 'DeepSeek Harness', 'HTTP-Referer': 'friendliconnect/v0.2.0' }
...
describe('extraHeaders on the wire', () => {
  it('merges config extraHeaders after the attribution headers', async () => {
    const fetchMock = vi.fn(async (_url: unknown, init?: { headers?: unknown }) =>
      new Response(
    ...
    try {
      ...
      expect(headers['HTTP-Referer']).toBe('friendliconnect/v0.2.0')
